...
CREATE OR REPLACE FUNCTION collector.public._final_median(anyarray)
   RETURNS float8 AS
$$ 
  WITH q AS
  (
...
CREATE AGGREGATE collector.public.median(anyelement) (
  SFUNC=array_append,
  STYPE=anyarray,
...
--drop table collector.public.job_data_filtered cascade;
--drop table collector.public.job_data_raw cascade;
--drop table collector.public.job_execution cascade;
--drop table collector.public.job cascade;
--drop table collector.public.job_parameter_value cascade;
--drop table collector.public.job_extraction cascade;
--drop table collector.public.job_filtering_filter cascade;
--drop table collector.public.job_filtering cascade;
--drop table collector.public.job_parameter cascade;
--drop table collector.public.filter cascade;
--drop table collector.public.job_execution_vw_mater cascade;
--drop table collector.public.ops_api_cached_calls cascade;

create table collector.public.job_extraction
(
	job_extraction_id serial PRIMARY KEY,
	job_extraction_description varchar(250)
);

--create table collector.public.job_parameter
--(
...
create table collector.public.job_parameter_value 
(
	...
	job_parameter_field varchar(250),
	job_parameter_value varchar(250)
);


create table collector.public.job_filtering
(
...
CREATE TABLE collector.public.filter
 (
...
create table collector.public.job_filtering_filter
(
...
--- v1.3
CREATE TABLE collector.public.activities_series
 (
   activities_series_id serial PRIMARY KEY,
   activities_series_description varchar(250),
   activities_count numeric(10),
   compounds_count numeric(10)
);

CREATE TABLE collector.public.activities 
 (
   activities_id serial PRIMARY KEY,
   activities_series_id integer references activities_series (activities_series_id) ON DELETE CASCADE,
   relation varchar(10),
...
create table collector.public.job 
(
	job_id serial PRIMARY KEY,
	job_description varchar(250),
	job_extraction_id integer references job_extraction (job_extraction_id) ON DELETE CASCADE,
	job_filtering_id integer references job_filtering (job_filtering_id) ON DELETE CASCADE,
	job_series_id integer references activities_series (activities_series_id) ON DELETE CASCADE --- v1.3
);

create table collector.public.job_execution 
(
...
create table collector.public.job_execution_step  
(
	job_execution_id integer,
 	filter_id integer,
 	job_step_id integer,
 	activities_series_id integer,
 	PRIMARY KEY (job_execution_id,job_step_id),
 	...
 	FOREIGN KEY (activities_series_id) REFERENCES activities_series (activities_series_id) ON DELETE CASCADE
)

----

CREATE TABLE collector.public.job_data_raw
 (
   job_data_raw_id serial,
   job_execution_id integer,   
   target_cwiki varchar(200) DEFAULT NULL,
   compound_cwiki varchar(200) DEFAULT NULL,--cambiado
   ...
   standard_units varchar(50),
   standard_value numeric(30,3),
   activity_value numeric(30,3),---nuevo
   activity_type varchar(100),
   inchi varchar(2000),
   inchikey varchar(2000),
   smiles varchar(1000),
   ...
   compound_pref_label varchar(10000),
   target_pref_label varchar(10000),---nuevo
   assay_description varchar(10000),---nuevo
   target_title varchar(10000),--nuevo
   sdf2d varchar(200000),
   sdf3d varchar(200000),
   PRIMARY KEY (job_data_raw_id),
   FOREIGN KEY (job_execution_id) references job_execution (job_execution_id) ON DELETE CASCADE
   
);

CREATE INDEX job_execution_id_raw ON collector.public.job_data_raw (job_execution_id);


CREATE TABLE collector.public.job_data_filtered
 (
...
CREATE INDEX job_execution_id_filt ON collector.public.job_data_filtered (job_execution_id);

create or replace view collector.public.job_data_raw_vw as
select draw.job_data_raw_id, 
   ...
   draw.target_title
FROM collector.public.job_data_raw draw;


CREATE OR REPLACE VIEW collector.public.job_data_filtered_vw AS 
...
CREATE OR REPLACE VIEW collector.public.job_data_filtered_vw_norm as 
select * 
...
--create or replace view collector.public.job_execution_stats_raw as
...
--create or replace view collector.public.job_execution_stats_filtered as
...
create or replace view collector.public.job_execution_stats as 
...
create or replace view collector.public.job_execution_vw as
...
CREATE TABLE collector.public.job_execution_vw_mater
(
  job_filtering_id integer,
...
CREATE OR REPLACE VIEW collector.public.job_data_statistics_base_vw AS 
 SELECT draw.job_data_raw_id, draw.job_execution_id,
...
CREATE OR REPLACE VIEW collector.public.job_data_filtered_vw_averaged AS 
...
CREATE OR REPLACE VIEW collector.public.job_data_raw_vw_averaged AS 
...
CREATE TABLE ops_api_cached_calls
(
  call character varying(500) NOT NULL,
  response text,
  "timestamp" timestamp with time zone,
  CONSTRAINT ops_api_cached_calls_pkey PRIMARY KEY (call )
);
